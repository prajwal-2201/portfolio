    ...
    >
      ...
      <div className="absolute top-16 left-10 md:left-20 z-20">
        <div className="flex items-center gap-3 mb-2">
          <Target size={16} className="text-amber-500/60" />
          <p className="text-[10px] text-amber-600/70 uppercase tracking-[0.4em] font-mono">Evidence Locker</p>
        </div>
        <h2 className="text-3xl md:text-5xl font-light text-white tracking-tight">
          Seized <span className="font-bold text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-cyan-400">Artifacts</span>
        </h2>
      </div>
      ...
      <div ref={scrollRef} className="flex gap-12 px-[15vw] mt-24">
        {missions.map((mission, idx) => (
          ...
          >
            {/* Evidence bag outer frame */}
            <div className="relative h-[70vh] bg-[#050505] border border-amber-900/30 rounded-2xl p-10 md:p-14 flex flex-col justify-between overflow-hidden transition-all duration-500 group-hover:border-amber-500/40 group-hover:shadow-[0_0_60px_rgba(245,158,11,0.06)]">
              ...
              <div className="absolute top-0 right-0 p-6 flex flex-col items-end gap-1">
                ...
                <div className="bg-amber-950/30 border border-amber-900/40 rounded px-3 py-1">
                  <span className="text-[9px] font-mono text-amber-600/70 uppercase tracking-widest">EVIDENCE #{mission.id}</span>
                </div>
              </div>

              {/* Tamper seal */}
              <div className={`absolute top-4 left-4 text-[9px] font-mono uppercase tracking-widest px-2 py-1 rounded border transition-all duration-500 ${hoveredIndex === idx ? 'border-green-500/60 text-green-500 bg-green-950/30' : 'border-amber-900/30 text-amber-700/50 bg-amber-950/10'}`}>
                {hoveredIndex === idx ? '✓ Seal Intact' : '⚠ Sealed'}
              </div>

              {/* Scanning Line Animation on hover */}
              <div className={`absolute top-0 left-0 w-full h-px bg-amber-500/30 z-10 transition-opacity duration-300 ${hoveredIndex === idx ? 'opacity-100 animate-scan' : 'opacity-0'}`} />
